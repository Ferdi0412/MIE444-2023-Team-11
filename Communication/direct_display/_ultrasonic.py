...
class UltrasonicSensor:
    ...
    def cx(self) -> float:
        ...
        return self._x0 + _math.sin(self._theta) * self._cy + _math.cos(self._theta) * self._cx

    @property
    def cy(self) -> float:
        """Calculate position of sensor based on given theta orientation."""
        return self._y0 + _math.sin(self._theta) * self._cx + _math.cos(self._theta) * self._cy

    @property
    def txt(self) -> str:
        """Return string of latest reating."""
        return f"{self.latest():.1f}"
    ...
    def draw(self, screen: _pygame.Surface) -> None:
        ...
        for ray_i in range(-STEPS, STEPS+1):
            ...
            x     = cx + dist * _math.sin(rot - alpha) # + theta
            y     = cy + dist * _math.cos(rot - alpha) # + theta
            _pygame.draw.line(screen, RAY_COL, (cx, cy,), (x, y,))

            ## Display average if SHOW AVERAGE
            if SHOW_AVG:
                x = cx + mean * _math.sin(rot - alpha) # + theta
                y = cy + mean * _math.cos(rot - alpha) # + theta
                _pygame.draw.line(screen, RAY_AVG_COL, (cx, cy,), (x, y,))
