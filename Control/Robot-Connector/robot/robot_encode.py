...
def encode_forward(dist: float, speed: float = 60.0) -> bytes:
    ...
    pulses: int = ( dist * _config.PULSES_FWD ) // 1
    ...
    msg: bytes = b'M' + _config.MOTOR_LEFT  + _encode_int( pulses ) + _encode_float( speed )\
               + b'M' + _config.MOTOR_RIGHT + _encode_int( pulses ) + _encode_float( speed )
...
def encode_rotate(angle: int, speed: float = 60.0) -> bytes:
    ...
    pulses: int = ( angle * _config.PULSES_ROT ) // 1
