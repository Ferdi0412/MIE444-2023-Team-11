...
### PRIVATE FUNCTIONS ###
#########################
def _generate_maze(resolution, distance_threshold):
    """Initialize MAZE."""
...
def distance_any_direction_raw(row, col):
    return MAZE_RAW[row, col]



def distance_any_direction(row, col):
    return MAZE[row, col]
...
########################
def setup():
    global MAZE, REPEATED, REPEAT_COUNTS, MAZE_RAW
    MAZE          = _generate_maze(RESOLUTION, DISTANCE_THRESHOLD)
    REPEATED      = _non_unique(MAZE)[1:] ## Drop the WALL value
    REPEAT_COUNTS = _non_unique_counts(MAZE)[1:]  ## Drop the WALL counts
    MAZE_RAW      = _generate_maze(RESOLUTION, max(*MAZE.shape))
